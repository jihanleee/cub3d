...
static void	get_lineinfo(t_vars *vars)
{
	if (vars->rinfo.side == 0)
		vars->rinfo.wall_x = vars->player_y + vars->rinfo.perpwalldist * vars->rinfo.raydir_y;
	else
		vars->rinfo.wall_x = vars->player_x + vars->rinfo.perpwalldist * vars->rinfo.raydir_x;
